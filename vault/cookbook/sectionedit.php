 ...
 * @author Charlie<cosmotic@cybercoment.com>
 * @author Said Achmiz <said@saidachmiz.net>
 * @version 2.4 (2019-05-06)
 * @link http://www.pmwiki.org/wiki/Cookbook/SectionEdit
 * @copyright by the authors 2017
 * @license http://www.gnu.org/copyleft/gpl.html GNU General Public License

 Hint: to have a working TOC - use SlimTableOfContents or AutoTOC
...
Requirements
------------
* Requires PmWiki 2.2.110 or higher
* Last tested on PmWiki 2.2.114-stable
...
$SectionEditLinkText sets the text of section edit links (default is '(Edit)').
...
fixed lots of errors caused by the migration to Markup_e
Copyright 2015 Charlie <cosmotic@cybercoment.com>
...
* 2.3.1 - 2017-08-17
** Made section edit link text customizable ($SectionEditLinkText)
* 2.3 - 2015-05-21
** Fixed to use Markup_e etc
* 2.2.3 - 2013-02-06 - Petko
** Copied PHSC() function from pmwiki.php to fix bugs with PHP 5.4
...
Markup('nosections', 'directives', '/\\(:nosections:\\)/i', "mu_nosections");
Markup('autosections', '>nosections', '/\\(:autosections\s*(\d)*:\\)/i', "mu_nosections");
Markup('noautosections', '>autosections', '/\\(:noautosections\s*(\d)*:\\)/i', "mu_noautosections");
Markup('nosectionsinincludes','directives','/\\(:nosectionsinincludes:\\)/i', "mu_nosectionsinincludes");
Markup('horzsections', '>nosections', '/\\(:horzsections\s*(\d)*:\\)/i', "mu_horzsections");
Markup('nohorzsections', '>horzsections', '/\\(:nohorzsections\s*(\d)*:\\)/i', "mu_nohorzsections");

function mu_nosections() { $GLOBALS['SectionEditDisable'] = true; }
function mu_autosections() { $GLOBALS['SectionEditWithoutHeaders'] = false; }
function mu_noautosections() { $GLOBALS['SectionEditWithoutHeaders'] = true; }
function mu_nosectionsinincludes() { $GLOBALS['SectionEditInIncludes'] = false; }
function mu_horzsections() { $GLOBALS['SectionEditHorzLines'] = true; }
function mu_nohorzsections() { $GLOBALS['SectionEditHorzLines'] = false; }
...
Markup_e('include', '>if',
  '/\\(:include\\s+(\\S.*?):\\)/i',
  "PRR(SectionEditIncludeText(\$pagename, \$m[1]))");
*******/
...
Markup_e('sectionedit','links',
  '/\\(:sectionedit\\s+(\\S.*?)\\s+(\\S.*?)\\s+(\\d.*?)\\s+(\\S.*?)\\s+(\w.*?):\\)/i',
  "mu_sectionedit");
function mu_sectionedit($m) {
  return SectionEditCreateLink($m[1],$m[2],$m[3],$m[4], $m[5]);
}
#                        $1          $2          $3          $4          $5
...
function SectionEditCreateLink($pagename, $type, $number, $from='', $title) {
        ...
        $editlink = FmtPageName("<a name='s$pagename".'_'."$number'></a><a href='\$PageUrl?action=edit&amp;s=".
                $number."&amp;auto=".
                ($SectionEditWithoutHeaders ? "n" : "y")."&amp;from=$from' title='".$title."'>".$SectionEditLinkText."</a>", $pagename);
#eMFix
...
***/
  ...
  if ($MarkupMarkupLevel) return $text;   // processing (:markup:)
...
#eMFix variables for flow control:
    global $GroupHeaderFooterDone, $MarkupMarkupLevel;
    global $SectionEditActualPage;
    global $SectionEditCounter;
...
/************** step 5: fill in form *******************/
  ...
  #eMFix: renamed
  $FmtV['$ChunkBefore'] = str_replace('"', '&quot;', str_replace('$', '&#036;', PHSC($chunkbefore, ENT_NOQUOTES)));
  $FmtV['$ChunkAfter'] = str_replace('"', '&quot;', str_replace('$', '&#036;', PHSC($chunkafter, ENT_NOQUOTES)));
  $FmtV['$PNum'] = $n;
...
function SectionEditMarkupEscape($text) {
  ...
  for ($i=0;$i < count($SectionEditEscapePattern); $i++){
    $out = preg_replace_callback("/$SectionEditEscapePattern[$i]/is",
    	function($m){
    		return  Keep($m[0]);
    	}, $out);
  }
...
function SectionEditMarkupRestore($text) {
  ...
  while ($count>$KeepStackStart){
    ...
    $key = $keys[$count];
    $value =$values[$count];
    $out = preg_replace_callback("/$KeepToken$key$KeepToken/",
...
function mu_SectionEditIncludeText($m){
  extract($GLOBALS['MarkupToHTML']);
  return PRR(SectionEditIncludeText($pagename, $m[1]));
}

function mu_SectionEditFirstTime($m){
  extract($GLOBALS['MarkupToHTML']);
  return SectionEditFirstTime($pagename,$m[0]);
}
...
if (($action == 'browse') || ($action == 'view')) {
        ...
        Markup('include', '>if',
           '/\\(:include\\s+(\\S.*?):\\)/i',
           "mu_SectionEditIncludeText");

        $horzline_match = '';
        if ($SectionEditHorzLines) $horzline_match = '|(----+)';
        /* only show Links for section editing when browsing */
        Markup_e('editsecmarkgen', '<if', '/^.*(\(:nl:\)|header:\\)|\n)((!{1,6})|(====+)'. $horzline_match .').*$/s',
        "mu_SectionEditFirstTime");
}
